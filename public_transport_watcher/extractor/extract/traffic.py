...
def fetch_api_data(line_id: str) -> dict:
    """
...
def extract_traffic_data(data: dict) -> pd.DataFrame:
    """
...
def extract_lines_data() -> pd.DataFrame:
    """
...
def process_traffic_data() -> pd.DataFrame:
    ...
    all_line_data = [extract_traffic_data(fetch_api_data(line_id)) for line_id in rail_line_ids if
                     fetch_api_data(line_id)]
    traffic_df = pd.concat(all_line_data, ignore_index=True) if all_line_data else pd.DataFrame()

    if traffic_df.empty:
        return pd.DataFrame()

    traffic_df = traffic_df.merge(rail_lines_df, left_on="line_ref", right_on="ID_Line", how="left",
                                  suffixes=('_df', '_lines'))
    return traffic_df[
        ["line_name", "line_ref", "statut", "destination", "expected_arrival_time", "TransportMode", "OperatorName",
         "NetworkName", "ShortName_GroupOfLines"]].rename(
