...
def _display_paris_map():
    ...
    map_data = DashboardHelpers.safe_execute_with_validation(
        "get_coordinates_heatmap",
        "Données de la carte",
        ["latitude", "longitude", "request_count"]
    )
...
def _create_stacked_bar_chart(top_stations):
    ...
    traces_config = [
        ('Départs', 'departures', '#ff6b6b'),
        ('Arrivées', 'arrivals', '#4ecdc4')
    ]
...
def _create_balance_scatter_chart(top_stations):
    ...
    if not top_stations.empty:
        ...
        fig.add_trace(go.Scatter(
            ...
            name='Équilibre parfait', showlegend=False
        ))
...
def _display_stations_analysis():
    ...
    stations_data = DashboardHelpers.safe_execute_with_validation(
        "get_response_stations",
        "Données d'analyse des stations",
    ...
    if search_station:
        filtered_data = filtered_data[
            filtered_data['station_name'].str.contains(search_station, case=False, na=False)
        ]
